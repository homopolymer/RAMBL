...
def map_data_to_seed_otus(seed_otus,data,mapper,map_args,out_dir,out_prefix,cores):
    ...
    for sample,runs in data.iteritems():
        for run in runs:
            ...
            logging.debug('elapsed time on mapping run {} of sample {} is {} minutes'.format(\
                          rn,sample,round((time.time()-t0)/60.,5)))
...
def recluster_data(fasta_file,otu_file,bam_file,mapper,map_args,cores,start_from,verbose):
    ...
    if start_from == 0:
        ...
        if verbose:
            logging.info('elapsed time on building the {} index file is {} minutes'.format(\
                         mapper,round((time.time()-t0)/60.,5)))
    ...
    out_prefix = os.path.join(read_dir,'marker_gene_reads')
    if start_from <= 1:
        extract_marker_gene_reads(bam_files,out_prefix,cores)
    single_reads = out_prefix + '.fastq'
    pe1_reads = out_prefix + '_1.fastq'
    pe2_reads = out_prefix + '_2.fastq'
    if file_size(single_reads)>0:
        sample_data.setdefault('all_samples',[]).extend([('single',single_reads)])
    if file_size(pe1_reads):
    ...
    if start_from <= 2:
        ...
        if verbose:
            logging.info('elapsed time on mapping reads onto seed otus is {} minutes'.format(\
...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument('-A','--map_args',help='mapping program arguments (default: --sensitive-local)',\
                            dest='map_args',default='--sensitive-local',type=str)
...
(default: 0)''',\
        ...
        # complete
        if opts.verbose:
            logging.info('total elapsed time is {} minutes'.format((time.time()-t_start)/60.))
        sys.exit(0)
